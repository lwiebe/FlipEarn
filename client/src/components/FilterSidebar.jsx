...
const FilterSidebar = ({ showFilterPhone, setShowFilterPhone, filters, setFilters }) => {
  ...
  return (
    <div className={`${showFilterPhone ? "max-sm:fixed" : "max-sm:hidden"} max-sm:inset-0 z-100 max-sm:h-screen max-sm:overflow-scroll bg-white rounded-lg shadow-md border border-gray-200 h-fit sticky top-24 md:min-w-[300px]`}>
            ...
            <div className='flex flex-col gap-2'>
              {platforms.map((platform) => (
                <label key={platform.value} className='flex items-center gap-2 text-gray-700 text-sm'>
                  <input type="checkbox"
                    checked={filters.platform?.includes(platform.value) || false} onChange={(e) => {
