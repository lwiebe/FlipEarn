...
// Controller for adding listing to db
...
// controller for updating listing in db
...
export const toggleStatus = async (req, res) => {
  try {
    ...
    if (listing.status === "active" || listing.status === "inactive") {
      ...
      return res.status(400).json({ message: "You listing is banned" });
...
export const deleteUserListing = async (req, res) => {
  try {
    ...
    // if password has been xhanged, send the new password to the owner
...
export const withdrawAmount = async (req, res) => {
  try {
    ...
    await prisma.user.update({
      where: { id: userId },
      data: { withdrawan: { increment: amount } }
    })
